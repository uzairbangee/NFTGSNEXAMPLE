...
const provider: any = (typeof window !== undefined) && new ethers.providers.Web3Provider((typeof window !== undefined) && window.ethereum, "any");
...
function Home() {
  ...
  const mintUsdc = async () => {
    setLoading(true);
    const node = (typeof document !== undefined) && document.querySelector("svg");
    if(name && node){
      const s = new XMLSerializer().serializeToString(node);
      const encodedData = (typeof window !== undefined) && window.btoa(s);
